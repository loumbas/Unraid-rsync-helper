...
- `src/emhttp/installed-checksums.txt` is a committed BUILD-TIME PLACEHOLDER; the real manifest
  (sha256+target+mode per file, version-stamped bytes, MANIFEST order, self-excluded) is
  GENERATED by the build into the .plg only (two-pass: a file cannot hash itself). Never
  hand-edit either. Doctor FAILs on missing/mismatched deployed files against it.
- There is no automated test suite; `tests/` holds the lint plus on-box acceptance records
  ...
  The lint also runs `shellcheck`/`php -l`/`bash -n` when available on the host.
...
- Every shipped file (src/engine, src/emhttp, src/jobs) must be listed in `src/MANIFEST` —
  ...
  mode `0NNN`. Adding a file = adding a MANIFEST line — insert it BEFORE the
  `emhttp/installed-checksums.txt` line, which must remain LAST (the checksum manifest is
  generated from every entry above it; both builders enforce the position).
...
- Scheduling: Unraid's Dillon cron reads only user crontabs, never `/etc/cron.d`. Jobs go
  ...
  when the block actually changed and must preserve Dynamix entries byte-for-byte. The block
  also carries a `*/15` watchdog maintenance line (history rollup compaction, log pruning,
  stale/stuck alerts) — removing it silently disables all retention.
- Live status: engine `sse_publish()` POSTs to `/pub/rclone-jobs` (nchan ships with Unraid 7;
  fire-and-forget, 2s curl cap, never fatal); WebUI subscribes via `EventSource('/sub/rclone-jobs')`
  and degrades to 60s polling on socket errors. `status-json` degrades a corrupt per-job status
  to null, never a broken response.
- `paths.env` retention knobs (`HISTORY_RAW_HOURS`, `HISTORY_RAW_MAX`, `HISTORY_HOUR_DAYS`,
  `HISTORY_DAYS`, `LOG_KEEP_DAYS`, `LOG_KEEP_FAIL_DAYS`, `LOG_KEEP_MAX`) are clamped by engine
  `num_clamp` — any UI edit path must mirror the clamp server-side (see Safety tab).
